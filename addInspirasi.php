...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($deskripsi)) {
        ...
        if (!$error) {
            ...
            $existingEventQuery = "SELECT * FROM inspirasi WHERE nama='$nama'";
            $existingEventResult = mysqli_query($koneksi, $existingEventQuery);

            if ($existingEventResult && mysqli_num_rows($existingEventResult) > 0) {
                // Update event
                $sql = "UPDATE inspirasi SET deskripsi='$deskripsi', gambar='$targetFile' WHERE nama='$nama'";
                $success = "Event berhasil diperbarui!";
            } else {
                // Insert new event
                $sql = "INSERT INTO inspirasi (nama, deskripsi, gambar) VALUES ('$nama', '$deskripsi', '$targetFile')";
    ...
    $nama = $_GET['nama'];
    $result = $koneksi->query("SELECT * FROM inspirasi WHERE nama='$nama'");
...
<head>
    ...
    <title><?php echo isset($_GET['nama']) ? 'Edit Event' : 'Tambah Event'; ?></title>
...
<body>
    <div class="container mt-4">
        ...
        <button type="click" name="submit" class="btn btn-primary"><a href="inspirasi.php">Kembali</a></button>
